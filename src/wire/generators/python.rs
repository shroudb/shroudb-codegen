...
fn gen_python_pipeline_method(out: &mut String, spec: &ProtocolSpec, cmd_name: &str, cmd: &CommandDef) {
    let method_name = cmd_name.to_snake_case();
    let positional = cmd.positional_params();
    let named = cmd.named_params();
    ...
    if !cmd.simple_response && !cmd.response.is_empty() {
        ...
        writeln!(out, "        self._commands.append((args, {response_type}._from_dict))").unwrap();
    } else {
