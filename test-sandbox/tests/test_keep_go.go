...
func main() {
	...
	err = client.Put("db/test/secret", value)
	check("put", err == nil)

	// 3. GET db/test/secret
	result, err := client.Get("db/test/secret", nil)
	...
	err = client.Put("db/test/secret", value2)
	check("put_v2", err == nil)

	// 5. GET db/test/secret VERSION 1
	resultV1, err := client.Get("db/test/secret", &shroudb_keep.GetOptions{Version: 1})
	check("get_v1", err == nil && resultV1 != nil)

	// 6. VERSIONS db/test/secret
	_, err = client.Versions("db/test/secret")
	...
	err = client.Delete("db/test/secret")
	check("delete", err == nil)

	// 9. Error: GET db/test/secret (deleted)
	_, err = client.Get("db/test/secret", nil)
